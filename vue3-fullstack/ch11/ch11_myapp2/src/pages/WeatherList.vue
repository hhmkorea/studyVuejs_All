<template>
  <div>
    ...
    <div v-if="store.loading" class="flex justify-center">
      <div class="loader ease-linear rounded-full border-8 border-t-8 border-gray-200 h16 w-16">
      </div>
    </div>
    <div v-else>
      <ul class="space-y-2">
        <li v-for="region in store.regions" :key="region.id" class="bg-white shadow rounded p-4 flex justify-between items-center">
          <router-link :to="`/weather/${region.id}`" class="text-lg font-medium text-blue-600 hover:underline">
            {{ region.name }}
          </router-link>
          <span :class="region.weather.main === 'Clear' ? 'text-yellow-500' : 'text-gray-700'">
            {{ region.weather.main }}
          </span>
        </li>
      </ul>
